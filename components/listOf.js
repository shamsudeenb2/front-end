...
const ListOf = ({
  ...
  return (
    <div
      className={`w-100 z-10 rounded-xl w-11/12 m-3 p-1  ${className}`}
      style={{ color: textColor }}
    >
      <div className="flex justify-between">
        <div className="">
          <p className={`font-bold text-xl `}> {doc_name}</p>
          <p className={`font-semibold pb-2`}>{reminder_type}</p>
          <p className={`font-semibold`}>{dosage_name}</p>
        </div>
        <div className="flex">
          <div className="">
            <i className=" bg-[#FFFFFF] flex-initial w-10">Edit</i>
          </div>
          <div className="">
            <svg
      ...
      <div class="flex">
        <Image src="/calender.png" alt="Calender" height={36} width={36} />
        <i className={`font-medium text-[#474646]`}></i>
        <i className="p-2">{date} </i>
      </div>
    </div>
  );
};
